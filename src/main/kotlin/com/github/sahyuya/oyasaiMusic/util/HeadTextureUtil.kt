 ...
 * プレイヤーヘッドのテクスチャを解決する。
 *
 * まずPaperのローカルプロフィールキャッシュを利用する。未キャッシュのJava版プレイヤーだけは PlayerDBのUUID
 * APIからスキンURLを取得し、MojangのプロフィールAPIは呼び出さない。 統合版（設定済み接頭辞の名前）は外部照会せず、スティーブ頭を表示する。
 */
object HeadTextureUtil {
  ...
  fun placeholderHead(uuid: UUID, name: String?): ItemStack =
      headItem(cachedProfile(uuid) ?: Bukkit.createProfile(uuid, name))

  /**
   * すぐにキャッシュ済みヘッド（未取得時はスティーブ頭）を返し、Java版の未キャッシュ分だけを PlayerDBから非同期で補完する。同一UUIDの同時要求は1回のHTTP通信に集約する。
   *
  ...
  private fun fetchPlayerDbProfile(request: SkinRequest): PlayerProfile? =
      runCatching {
            val httpRequest =
                HttpRequest.newBuilder(URI.create("$PLAYER_DB_ENDPOINT${request.uuid}"))
            ...
            val player =
                JsonParser.parseString(response.body())
                    ...
                    ?.getAsJsonObject("player") ?: return@runCatching null
            val skinUrl = player.get("skin_texture")?.asString ?: return@runCatching null
            val uri = URI.create(skinUrl)
            if (uri.scheme != "https" || uri.host != "textures.minecraft.net")
  ...
  private fun headItem(profile: PlayerProfile): ItemStack =
      ItemStack(Material.PLAYER_HEAD).also { item ->
