...
/**
 * いいね報酬（UI/UX設計書 7章「基本報酬」）を担当するサービス。 「無条件で即時（オフライン時はプール）発生」との記述通り、実際のVault送金は
 * ここでは行わず、[UserRepository] のpending残高へ積み立てる（受取操作はGUIフェーズで実装する メインメニューの「未受取報酬の一括受取」から行う）。
 *
