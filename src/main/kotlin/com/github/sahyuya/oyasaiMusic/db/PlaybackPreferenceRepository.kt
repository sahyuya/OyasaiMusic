 ...
 *
 * 追加項目.txt: 「立体音響再生は楽曲詳細GUIを開いた際に、個々のリスナーがその音楽を通常 (デフォルト)再生か立体音響再生かを選べて、その再生方法の選択を保存する。」に対応する。
 */
class PlaybackPreferenceRepository(private val db: DatabaseManager) {

  /** 保存済みの選択を取得する。未設定の場合はnull（呼び出し側でデフォルトにフォールバックする）。 */
  fun getMode(userUuid: UUID, songId: Long): PlaybackMode? =
      db.transaction { conn ->
        conn
            .prepareStatement(
  ...
  fun setMode(userUuid: UUID, songId: Long, mode: PlaybackMode) =
      db.transaction { conn ->
        conn
            .prepareStatement(
