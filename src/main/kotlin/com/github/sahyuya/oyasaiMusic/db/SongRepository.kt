...
class SongRepository(private val db: DatabaseManager) {
  ...
  fun updateStatus(id: Long, status: SongStatus) =
      db.transaction { conn ->
  ...
  fun setPublishedAndClaimFirstAnnouncement(id: Long, published: Boolean): Boolean =
      db.transaction { conn ->
        ...
        val firstPublish =
            conn.prepareStatement("SELECT first_published_at FROM songs WHERE id = ?").use { ps ->
              ps.setLong(1, id)
  ...
  fun updateAudioProperties(id: Long, supportsPositional: Boolean) =
      db.transaction { conn ->
