 ...
 *
 * @param recordMaterial 楽曲ランキングのみ設定される（バニラのレコード素材名）。 フォロワーランキング(FOLLOWERS)はnull。メインメニューのアイコン表示に使う
 ...
 * そのまま利用しており、build.gradle.ktsへの追加依存は不要な想定。 タイムゾーンはサーバーのシステムデフォルト([ZoneId.systemDefault])を使用するため、
...
class RankingCacheService(
  ...
  fun start() {
    ...
    Bukkit.getScheduler()
        .runTaskAsynchronously(
            plugin,
            Runnable { checkBoundariesAndRecompute(forceTotal = totalSnapshot == null) },
        )
    ...
    Bukkit.getScheduler()
        .runTaskTimerAsynchronously(
            plugin,
            Runnable { checkBoundariesAndRecompute(forceTotal = false) },
            20L * 60,
            20L * 60,
        )
    // 総合ランキングは30分ごとに再集計。
  ...
  private fun buildSnapshot(since: Long?, until: Long?, label: String): RankingSnapshot {
          ...
          if (metric == RankingMetric.FOLLOWERS) {
            rankingRepository.topAuthorsInRange(since, until, limit = 7).mapIndexed { i, a ->
              ...
              RankingEntryDto(
                  ...
                  songId = null,
                  title = null,
                  authorUuid = a.authorUuid.toString(),
                  authorName = name,
                  score = a.score,
              )
            }
    ...
    return RankingSnapshot(
        generatedAtEpochSec = System.currentTimeMillis() / 1000,
        periodLabel = label,
        entries = entries,
    )
  }
  ...
  private fun load() {
    try {
      if (cacheFile.exists()) {
        state =
            gson.fromJson(cacheFile.readText(), RankingCacheFile::class.java) ?: RankingCacheFile()
      }
    } catch (e: Exception) {
