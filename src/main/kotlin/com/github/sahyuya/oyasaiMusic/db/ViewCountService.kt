 ...
 *
 * 視聴ポイント（10回ごとに作者へ+1pt）は「収益化条件（参考URL登録 or 審査通過）」を 満たす楽曲のみ付与する（UI/UX設計書 7章）。総視聴回数(songs.views)自体は
 * 収益化条件を問わず加算する。
 *
 * GUIフェーズで追加: [onRegistered] コールバック。視聴回数が実際にDBへ記録できた場合のみ
 * メインスレッドで呼ばれる。「再生を2回行っても再生回数の反映が画面に表示されない」という 報告への対応で、呼び出し側(GUI画面)がこれを使って現在の画面を再描画できるようにするため
...
class ViewCountService(
  ...
  /**
   * 視聴条件を満たした際に呼び出す。内部でDBアクセスを非同期化するため、 呼び出し元のスレッド（メインスレッド想定）をブロックしない。
   *
   * @param onRegistered 視聴回数が実際に加算できた場合のみメインスレッドで呼ばれる （AFK・上限到達等でスキップされた場合は呼ばれない）
   */
  fun registerView(
      player: Player,
      song: Song,
      isAmbientPlayback: Boolean,
    ...
    Bukkit.getScheduler()
        .runTaskAsynchronously(
            ...
            Runnable {
              ...
              if (
                  !socialRepository.registerQualifiedView(
