...
class DemoSoundCommand : CommandExecutor, TabCompleter {
    ...
    override fun onCommand(sender: CommandSender, command: Command, label: String, args: Array<out String>): Boolean {
        ...
        val playbackPitch = notePitch?.let { InstrumentMapper.pitchToPlaybackPitch(it.toByte()) } ?: 1.0f
        val id = "${definition.idPrefix}:$pattern"

        sender.playSound(sender.location, selection.eventKey, SoundCategory.RECORDS, 1.0f, playbackPitch, selection.seed)
        sender.sendMessage("§a試聴: §f${definition.eventKey} §7(パターン $pattern${notePitch?.let { ", ピッチ $it" } ?: ""})")
        sender.sendMessage(
            Component.text("ID ", NamedTextColor.YELLOW)
                .append(
                    Component.text(id, NamedTextColor.AQUA)
                        .hoverEvent(HoverEvent.showText(Component.text("クリックでコピー", NamedTextColor.GRAY)))
    ...
    override fun onTabComplete(
        ...
        1 -> VanillaSoundCatalog.eventKeys().filter { it.contains(args[0].removePrefix("minecraft:"), ignoreCase = true) }
        2 -> VanillaSoundCatalog.find(args[0])?.availablePatterns()?.map(Int::toString)?.filter { it.startsWith(args[1]) } ?: emptyList()
