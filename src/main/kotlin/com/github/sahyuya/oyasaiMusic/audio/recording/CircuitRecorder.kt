...
object CircuitRecorder {
    ...
    /**
     * [isStrong] は「この位置の不透過ブロックが強動力を受けている」ことを表す。
     * 強動力は周囲のダストを起動でき、ダストによる弱動力はブロックを起動しても
     * そこから周囲のダストへ再伝播しない。
     */
    private data class Signal(
        ...
        override fun compareTo(other: Signal): Int = compareValuesBy(
            this,
            other,
            Signal::timeMs,
            { -it.power },
    ...
    fun record(clipboard: Clipboard, world: World): List<NoteEvent> {
        ...
        /**
         * リピーターの遅延完了時に、本体と正面の両方を処理する。
         * 正面にある不透過ブロックは強動力となる一方、本体側はノートブロック等を
         * 直接起動するための部品出力として扱う。これにより、FAWEコピー内で
         * リピーターの上・側面に配置された音ブロックも遅延後の時刻で記録できる。
         */
        ...
        val rescueNotes = if (notes.isEmpty() || notes.size * 2 < noteBlockCount) structuralFallback(
            sources = sources,
            blocks = blocks,
            wires = wires,
            repeaters = repeaters,
        ...
        return recordedNotes.sortedBy { it.timeMs }.map { note ->
            NoteEvent(
    ...
    /**
     * 接続面のBlockDataが失われたクリップボード用の救済経路。
     * 同じ水平面または一段差にあるダストを候補にし、リピーターだけは必ず背面入力から
     * 正面出力へ遷移させる。通常経路が0音符のときだけ使うため、正しい接続面を持つ回路の
     * 分岐判定や強弱動力の結果には影響しない。
     */
    ...
    private fun toRawNote(world: World, pos: BlockVector3, data: BukkitNoteBlock, timeMs: Int): RawNote {
    ...
    private fun connectedWireNeighbors(
        ...
        HORIZONTAL_FACES.forEach { face ->
            ...
            if (lowerWire != null && (!hasConnectionInfo || lowerWire.getFace(face.oppositeFace) == RedstoneWire.Connection.UP)) {
                add(lower)
            }
        }
