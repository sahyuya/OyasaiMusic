...
/**
 * `plugins/OyasaiMusic/soundeffect/` を効果音の拡張ポイントとして扱うサービス。 初回起動では JAR
 * 内の標準音源を展開し、以後はデータフォルダ側のファイルを優先するため、 サーバー運営者は同じファイル名の OYMB `.bin` を差し替えるだけで効果音を変更できる。
 */
  ...
  fun play(effect: PluginSoundEffect, recipients: Collection<Player>) {
    ...
    Bukkit.getScheduler()
        .runTaskAsynchronously(
            ...
            Runnable {
              val audio =
                  runCatching { SongAudioFile.read(file) }
                      .getOrElse {
