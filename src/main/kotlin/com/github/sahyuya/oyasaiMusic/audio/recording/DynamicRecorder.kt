...
/**
 * 生演奏録音（`/record live`）と現地回路録音（`/record we start`）の共通変換処理。 `NotePlayEvent` をフックする都度、[process]
 * を呼び出して1音符分の [NoteEvent] を得る。
...
object DynamicRecorder {
  ...
  fun processLiveCircuit(
    ...
    if (block.world.uid != session.worldUuid || !session.contains(block.x, block.y, block.z))
        return null
    val actualElapsedMs =
        ((eventTimeNanos - session.startTimeNanos).coerceAtLeast(0L) / 1_000_000L).toInt()
