 ...
 *
 * version 2では固定8バイトの直後に、カスタム音源パスの長さ(Short)とUTF-8文字列を続ける。
 * version 3ではさらにパターン固定用seed(Long)を続ける。長さ0は通常のノートブロック音色を表す。
 * version 1/2の既存ファイルはそのまま読み込める。
 */
object SongAudioFile {
    ...
    fun read(file: File): SongAudio {
        ...
        DataInputStream(BufferedInputStream(file.inputStream())).use { input ->
            ...
            require(version in 1..CURRENT_VERSION.toInt()) { "未対応の音源フォーマット(version=$version): ${file.name}" }
            ...
            repeat(totalNotes) {
                ...
                val customSoundSeed = if (version >= 3) input.readLong().takeIf { customSound != null } else null
                notes += NoteEvent(
                    timeMs = timeMs,
                    instrument = inst,
