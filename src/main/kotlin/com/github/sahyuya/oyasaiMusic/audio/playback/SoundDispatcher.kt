 ...
 *
 * [PlaybackMode.DEFAULT] (デフォルト再生): Adventure APIの `Audience#playSound(Sound, Sound.Emitter)` に
 * `Sound.Emitter.self()` を渡すことで、
 ...
 *
 * [PlaybackMode.POSITIONAL] (立体音響再生): 従来通り `Player#playSound(Location, Sound, SoundCategory, float,
 * float)` を、プレイヤーの 視線（Yaw+Pitchの3次元方向）を基準とした半円弧上の仮想座標に対して呼び出す方式。
...
object SoundDispatcher {
  ...
  private fun playDefault(recipient: Player, note: NoteEvent) {
    ...
    val adventureSound =
        AdventureSound.sound(bukkitSound, AdventureSound.Source.RECORD, volume, pitch)
  ...
  /**
   * POSITIONAL方式専用。プレイヤーの視線（Yaw+Pitchの3次元方向）を正面(Pan=0)として、 左右90度ずつに広がる半径1ブロックの半円弧上に仮想音源を配置する。
   * Pan=0は正面（上下の視線にも追従する）、Pan=±100は水平方向の左右90度で固定する （sin(90°)=1, cos(90°)=0
   * となるため、正面ベクトルのY成分は自動的に打ち消される）。
   */
  private fun virtualLocation(player: Player, pan: Int): Location {
    ...
    return eye.clone()
        .add(dirX * SOURCE_RADIUS_BLOCKS, dirY * SOURCE_RADIUS_BLOCKS, dirZ * SOURCE_RADIUS_BLOCKS)
  }
}
