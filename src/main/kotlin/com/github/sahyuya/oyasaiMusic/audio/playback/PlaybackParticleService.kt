 ...
 *
 * 個人再生は再生している本人の頭上だけに表示する。環境レコードは聴取者へは表示せず、
 * 音源であるジュークボックスの上だけに表示することで、誰が再生しているかを混同させない。
 */
class PlaybackParticleService(private val plugin: OyasaiMusic) {
    ...
    fun tick() {
        plugin.controllerStateService.activeStates().forEach { (playerUuid, state) ->
            ...
            if (!player.isOnline || !state.isPlaying || session.isCancelled || session.isPaused) return@forEach
