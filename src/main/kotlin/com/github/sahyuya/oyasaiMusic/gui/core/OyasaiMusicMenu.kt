...
/** OyasaiMusicのGUI画面が実装する共通契約。 [MenuManager]はこの型を通じてクリック・再描画・履歴管理を扱う。 */
...
/** 54スロット（6×9）の標準GUI基底クラス。 左列ナビゲーション・下段コントローラー・5×8コンテンツ領域を持つ画面で使用する。 */
abstract class BaseGridMenu(
    final override val viewer: Player,
    title: Component,
) : OyasaiMusicMenu {
  final override val inventory: Inventory =
      Bukkit.createInventory(OyasaiMusicMenuHolder(this), 54, title)
}
